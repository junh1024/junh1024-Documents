...
* You can change your pitchshifter/settings & balance sound quality & CPU. Pitchshifting vocals require formant awareness/control for >1st changes to sound good. If you're not making large changes to vox, you can prolly use Elastique Soloist Speech to save CPU. I think it some formant correction (but settings are not exposed). OTOH, if you're making larger changes, you may need to use the default Elastique Pro mode (medium CPU) &/ change the formant shift too. Or maybe you can try the different Elastique Pro Preserve Formants variations (high CPU) to make it sound more natural. For large changes to instros, you can still try  Elastique Pro Preserve Formants as a high-CPU alternative to an EQ boost.
...
Chord theory
---
Sometimes you may need to pitchshift loops to get the right chord.
